...
for xi in range(Nx):
	for yi in range(Ny):
		#Jx0[int((xi+Nx/2)%Nx)][yi] = 0.5*( jx0[xi][yi] + jx0[xi+1][yi] )
		#Jy0[int((xi+Nx/2)%Nx)][yi] = 0.5*( jy0[xi][yi] + jy0[xi][yi+1] )
		#Jx1[int((xi+Nx/2)%Nx)][yi] = 0.5*( jx1[xi][yi] + jx1[xi+1][yi] )
		#Jy1[int((xi+Nx/2)%Nx)][yi] = 0.5*( jy1[xi][yi] + jy1[xi][yi+1] )
...
#plt.quiver(X,Y,Jx1.T,Jy1.T, color="red", minlength=1.)
